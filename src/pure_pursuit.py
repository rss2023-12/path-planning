...
class PurePursuit(object):
    ...
    def trajectory_callback(self, msg):
        ...
        int1_angle = np.arctan2((y1_int - y_car), (x1_int - x_car))
        ...
        pursue_angle = np.arctan2((pursue_point[1] - y_car), (pursue_point[0] - x_car))
        pursue_dir = np.array([np.cos(pursue_angle), np.sin(pursue_angle)])
        eta = np.arccos(np.dot(v_ref, pursue_dir)/(a))        # a is L_1 which is the lookahead radius
        ...
        drive_cmd.drive.steering_angle = np.arctan2(2*L*np.sin(eta), L_1)
